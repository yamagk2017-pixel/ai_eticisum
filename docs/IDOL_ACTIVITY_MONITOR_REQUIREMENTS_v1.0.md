...
運用意図（確定）:
- 候補抽出では YouTube / Spotify の更新を積極的に扱う。
- 理由は、配信導線（動画視聴・音源再生）に直結する活動シグナルを優先的に観測し、配信起点の収益化支援につなげるため。
...
- 補完結果は `weekly_group_complements` に保存し、`/relay-9147/iam` に「AI補足」として表示する
- 料金ガード:
  - 日次上限: `$2.5`
  - 月次上限: `$10`
  - 上限到達時は補完結果として `利用限度（料金）の上限に達しました` を表示する

### 17.5 Responses API 指示仕様（実装反映）
- 対象は日本の女性アイドルグループ文脈に限定する
- 通常補足は「過去1週間の活動状況 + 今後1週間の活動予定」を対象に整理する
- 重大ニュースは「基準日以降〜今後6か月以内に予定・発表されているもの」を対象にする
- `major_ongoing_topics` は「今後6か月以内の重大予定」を最大3件返す
- 出力はJSON形式（`summary`, `bullets`, `major_ongoing_topics`, `sources`）
- `slug` は外部検索ノイズとなるため、プロンプト入力から除外する
...
- 例: `/relay-9147/iam/targets`, `/relay-9147/iam/activities`, `/relay-9147/iam`
...
- グループ詳細（ナンダッテ）へのリンク
- グループの公式Xリンク（`external_ids` に存在する場合）
- 補足情報の参照URLを実リンクで表示
...
- `weekly_group_complements` の基準週は `weekly_digest_candidates` の最新 `week_key` を参照する。
...
## 27. 実装状況（2026-04-20時点）

### 27.1 実装済み
- IAM画面（`/relay-9147/iam/targets`, `/relay-9147/iam/activities`, `/relay-9147/iam`）
...
- `weekly_group_complements` 生成（OpenAI Responses API + Web検索補完）
  ...
  - 基準週: `weekly_digest_candidates` の最新 `week_key`
  - 料金ガード: 日次 `$2.5` / 月次 `$10`
  - 上限時の表示文言: `利用限度（料金）の上限に達しました`
  - 表示先: `/relay-9147/iam` の「AI補足」
- `run-weekly` のフォールバック
  - YouTube/Spotify収集が失敗（例: API 403）しても、ジョブ全体は継続して後続処理を実行
- 候補UI改善（`/relay-9147/iam`）
  - GROUPにナンダッテ詳細リンク、公式Xリンク（存在時）を表示
  - HEADLINEにYouTube/Spotify種別ラベルを表示
  - AI補足の参照URLをリンク一覧で表示
  - 壊れたJSON文字列として保存されたAI補足は、可能な範囲で整形して表示
