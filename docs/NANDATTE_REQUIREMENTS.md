...
- 全てのページに「いいね」機能を設ける。
- いいね可能なのはログインユーザーのみ。
- いいね数に基づくランキングページを生成する。
- ランキング対象となるいいねの有効期間は **過去1週間（7日間）** とする。
- トップページに、いいねランキング **TOP5** を表示する。

### 10.2 管理者おすすめ機能
- 管理者権限で、おすすめしたい曲・記事・ページを設定できる。
- 管理者が設定したおすすめ項目をトップページに表示する。

### 10.3 デザインモード（テーマ方針）
- デザインの表示モードとして、ダークモード／ライトモードを用意する。
- ただしライトモードの呼称は **「ポップモード」** とする。
- ポップモードは「単に明るい配色」ではなく、**親しみやすいUI** を目指して設計・開発する。

### 10.4 管理者への改善リクエスト機能（検討）
- ログインユーザー限定で、管理者へ簡易的な改善リクエストを送れる機能を検討する。
- 想定用途：ナンダッテ掲載中のプロフィール情報が古い場合などの更新依頼。
- 自由記述の長文メッセージ送信ではなく、**選択式 + ショートメッセージ** 形式を基本とする。
- 例：`「情報を更新してほしい」` などの短い定型メッセージ付き選択肢を用意し、選んで送信できるようにする。
- 目的は、ユーザーの送信ハードルを下げつつ、管理者側で処理しやすい依頼フォーマットにすること。
- 受信方式の初期方針は、**DB保存（正本） + メール通知** を基本とする（件数が少ない想定のため）。
- メールは通知用途とし、送信内容の正本データはDBに保持する。
- メールの見落とし対策として、管理者向けに**簡易一覧ページ（最低限の管理UI）** を用意する方針を検討する。
- 初期の管理ステータスは `未対応 / 完了` の最小構成を想定し、必要に応じて将来拡張する。
...
- フリーワードの扱い：
  ...
  - 不要フリーワード削除の運用方針：
    - 当面は「投票更新時の即時クリーンアップ（孤立freeword削除）」を採用
    - 将来的に「定期Cronでの0票フリーワード一括削除」を追加検討（ペンディング）
- 「1グループ1回」厳密性の担保：
