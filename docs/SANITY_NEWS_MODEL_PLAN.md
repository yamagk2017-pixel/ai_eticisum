...
- `seoDescription`
- `ogImage`
- `relatedGroups`（`imd.groups` 連携用の構造化フィールド）
...
- `tags`（references[]）
- `relatedGroups`（array, `imd.groups`連携。複数可だが原則1件）
- `excerpt`（text, 任意・低優先。本文冒頭で代替可）
- `seoTitle`（string）
- `seoDescription`（text）
...
## `imd.groups` 連携フィールド（重要要件）

ハッシュタグとは別に、記事が扱うグループを `imd.groups` の正規マスタと紐付けるための専用入力項目を持つ。

### 目的

- 記事詳細で `imd.groups` の関連情報を表示するため
- タグ（自由入力・表記揺れあり）とは別に、マスタ連携された正規データを持つため
- `newsArticle` / `wpImportedArticle` の両方で同じ関連グループ表示ロジックを使えるようにするため

### フィールド方針

- フィールド名: `relatedGroups`
- 対象スキーマ:
  - `newsArticle`
  - `wpImportedArticle`
- 型:
...
- 運用ルール:
  - 基本は `1記事 = 1グループ`
  - 例外的な記事のみ複数登録を許容
  - 理由: 関連情報の表示がグループ数分増え、情報過多になりやすいため

### 入力UX（Sanity Studio）

入力フォームでは `id` や `slug` を直接入力させず、`グループ名` を入力して候補選択する方式を採用する。

想定UI:
- テキストボックスにグループ名を入力
- `imd.groups.name_ja` から候補を検索
- 候補一覧を表示（Autocomplete / Typeahead）
- 候補を選択して `relatedGroups` に追加

理由:
- `id` / `slug` は覚えづらく、入力ミスが起きやすい
- 日本語のグループ名で探せる方が運用しやすい

### 保存値（内部）

表示は `name_ja` ベースだが、保存は `imd.groups` の内部キー（`imdGroupId`）を基本とする。
...
必要に応じて追加候補:
- `displayOrder`（複数登録時の表示順）
- `groupNameSnapshot`（デバッグ/監査補助、任意）
...
- 公開日時を載せる
- `og:type` は `article`
- `description` は `本文冒頭` を基本にし、`excerpt` は任意（あれば使う程度）
...
5. `relatedGroups` のSanity Studio実装方式
- custom input component（Autocomplete）で実装するか
- `imd.groups` 候補取得APIの提供方法をどうするか
- 着手メモ: `src/sanity/components/related-groups-input/related-groups-input-plan.md`
