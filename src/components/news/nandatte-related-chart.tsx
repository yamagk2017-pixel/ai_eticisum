...
export function NandatteRelatedChart({groups}: Props) {
  ...
  return (
    <section className="mb-14 rounded-2xl border border-zinc-500/60 px-4 pb-10 pt-6 sm:px-6 sm:pb-8">
      <div className="flex flex-col gap-2">
        <h2 className="font-mincho-jp text-xl font-medium leading-tight text-zinc-900 sm:text-2xl">
          {active.groupNameJa}ってこんなグループ「ナンダッテ」
        </h2>
        <p className="text-xs text-zinc-700">ナンダッテ投票の上位ワード（上位5件）</p>
      </div>
            ...
            <button
              ...
              className={`text-sm underline-offset-4 transition ${
                index === safeActiveIndex
                  ? "font-medium text-zinc-900 underline decoration-zinc-500"
      ...
      {status === "loading" ? (
        <p className="mt-6 text-sm text-zinc-700">棒グラフを読み込み中...</p>
      ) : status === "error" ? (
        <p className="mt-6 text-sm text-zinc-700">棒グラフを取得できませんでした。{errorMessage ? ` (${errorMessage})` : ""}</p>
      ) : (data?.items.length ?? 0) === 0 ? (
        <p className="mt-6 text-sm text-zinc-700">まだ投票がありません。</p>
      ) : (
        <div className="mt-5 grid grid-cols-2 gap-x-4 gap-y-2 rounded-xl border border-zinc-300 bg-transparent p-3 sm:gap-x-5">
          {data!.items.slice(0, 5).map((item, index) => {
            ...
            return (
              <div key={`${item.label}-${index}`} className="px-1 py-2">
                <div className="flex items-center justify-between gap-4 text-sm text-zinc-800">
                ...
                <div className="mt-2 h-4 w-full bg-zinc-200">
                  <div
                    className={`h-4 ${isTopFive ? "bg-zinc-600" : "bg-zinc-500"}`}
                    style={{width: `${width}%`}}
                  />
                </div>
              </div>
      ...
      <div className="mt-5">
        {nandattePath ? (
          <a
            href={nandattePath}
            className="inline-flex items-center text-sm text-zinc-800 underline decoration-zinc-500/70 underline-offset-2"
          >
            <span>{active.groupNameJa}をもっと詳しく</span>
            <span aria-hidden="true">→</span>
          </a>
        ) : (
          <p className="text-xs text-zinc-700">ナンダッテ詳細ページのリンクは未登録です。</p>
        )}
      </div>
    </section>
  );
