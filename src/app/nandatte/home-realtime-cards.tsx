...
type HomeNandatteRealtimeCardsProps = {
...
export function HomeNandatteRealtimeCards({
  initialVoteTop,
  initialRecentTop,
  pollIntervalMs = 15000,
}: HomeNandatteRealtimeCardsProps) {
  ...
  useEffect(() => {
    ...
    const fetchLatest = async () => {
      try {
        const res = await fetch("/api/nandatte/rankings?limit=3", {
          method: "GET",
          cache: "no-store",
        });
